...
**In VS Code Copilot:** Skills are discovered automatically via their `description` field in YAML frontmatter. When a skill matches the current task, its content is loaded into context — follow it directly. Skills also appear as `/` slash commands in chat. Place skills in `~/.agents/skills/<name>/` (personal) or `.github/skills/<name>/` (project-level).

## Platform Adaptation

This Copilot-adapted version uses VS Code Copilot tool names natively. For cross-platform reference, see `references/copilot-tools.md`.
